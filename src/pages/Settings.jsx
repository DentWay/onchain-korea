        ...
        <section className="bg-[var(--app-paper-bg)] border border-[var(--app-paper-border)] rounded-[2rem] shadow-[0_4px_24px_rgba(0,0,0,0.03)] p-5 md:p-8">
          ...
          <header className="mt-5 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-2xl">
              <p className="text-[11px] uppercase tracking-[0.2em] text-[var(--app-ink-low)]">{pick(lang, '내 정보', 'My Account')}</p>
            ...
            <div className="bg-[var(--app-paper-muted-bg)] border border-[var(--app-paper-border)] rounded-[1.5rem] px-5 py-4 md:px-6 md:py-5 lg:min-w-[250px]">
              <p className="text-[11px] uppercase tracking-[0.18em] text-[var(--app-ink-low)]">{pick(lang, '현재 계정', 'Current account')}</p>
              <div className="mt-3 flex items-center gap-3">
                <div className="flex h-12 w-12 items-center justify-center rounded-full bg-[var(--app-light-btn-bg)]">
                  <UserCircle2 size={22} className="text-[var(--app-ink-mid)]" />
                </div>
                <div className="min-w-0">
                  <p className="truncate text-[18px] font-[800] tracking-[-0.04em] text-[var(--app-ink-high)]">{currentDisplayName || '-'}</p>
                  <p className="truncate text-[12px] text-[var(--app-ink-mid)]">{user?.email || '-'}</p>
                </div>
              </div>
            </div>
          </header>
          ...
          <div className="mt-6 grid gap-5 xl:grid-cols-[minmax(0,1.2fr)_320px]">
            <div className="space-y-5">
              <section className="bg-[var(--app-paper-bg)] border border-[var(--app-paper-border)] rounded-[1.5rem] overflow-hidden">
                <div className="border-b border-[var(--app-paper-border)] px-5 pb-4 pt-5 md:px-6 md:pt-6">
                  <h2 className="text-[20px] font-[800] tracking-[-0.04em] text-[var(--app-ink-high)]">{pick(lang, '프로필', 'Profile')}</h2>
                ...
                <form onSubmit={handleSave} className="space-y-5 px-5 py-5 md:px-6 md:py-6">
                  <div className="space-y-2">
                    <label htmlFor="display-name" className="text-[12px] font-semibold text-[var(--app-ink-high)]">
                    ...
                    <input
                      ...
                      className="w-full rounded-2xl border border-[var(--app-paper-border)] bg-[var(--app-paper-bg)] px-4 py-3 text-[14px] text-[var(--app-ink-high)] outline-none transition-colors placeholder:text-[var(--app-ink-low)] focus:border-[#5741d8]"
                      maxLength={24}
                    ...
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <span className={`text-[12px] tabular-nums ${isDisplayNameValid ? 'text-[var(--app-ink-low)]' : 'text-[var(--error)]'}`}>
                  ...
                  <div className="space-y-2">
                    <label htmlFor="email-address" className="text-[12px] font-semibold text-[var(--app-ink-high)]">
                      {pick(lang, '이메일', 'Email')}
                    </label>
                    <input
                      id="email-address"
                      value={user?.email || ''}
                      readOnly
                      className="w-full rounded-2xl border border-[var(--app-paper-border)] bg-[var(--app-paper-muted-bg)] px-4 py-3 text-[14px] text-[var(--app-ink-mid)] outline-none"
                    />
                  </div>
                    ...
                    <div
                      className={`rounded-2xl border px-4 py-3 text-[13px] ${
                        saveState.status === 'success'
                          ? 'border-[rgba(20,158,97,0.18)] bg-[rgba(20,158,97,0.10)] text-[var(--success)]'
                          : 'border-[rgba(220,38,38,0.20)] bg-[rgba(220,38,38,0.08)] text-[var(--error)]'
                      }`}
                    >
                      {saveState.message}
                    </div>
              ...
              <section className="bg-[var(--app-paper-bg)] border border-[var(--app-paper-border)] rounded-[1.5rem] overflow-hidden">
                ...
                <div className="space-y-5 px-5 py-5 md:px-6 md:py-6">
                  <div className="space-y-2">
                    <label htmlFor="delete-phrase" className="text-[12px] font-semibold text-[var(--app-ink-high)]">
                    ...
                    <input
                      ...
                      className="w-full rounded-2xl border border-[rgba(220,38,38,0.20)] bg-[rgba(220,38,38,0.04)] px-4 py-3 text-[14px] text-[var(--app-ink-high)] outline-none transition-colors placeholder:text-[var(--app-ink-low)] focus:border-[var(--error)]"
                  ...
                  {deleteState.message && (
                    <div className="rounded-2xl border border-[rgba(220,38,38,0.20)] bg-[rgba(220,38,38,0.08)] px-4 py-3 text-[13px] text-[var(--error)]">
                      {deleteState.message}
                    </div>
                  )}
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded-full border border-[rgba(220,38,38,0.24)] bg-[rgba(220,38,38,0.08)] px-5 py-3 text-[13px] font-semibold text-[var(--error)] transition-colors hover:bg-[rgba(220,38,38,0.12)] disabled:cursor-not-allowed disabled:opacity-50"
                  >
              ...
              <div className="bg-[var(--app-paper-muted-bg)] border border-[var(--app-paper-border)] rounded-[1.5rem] p-5">
                ...
                <dl className="mt-4 space-y-3">
                  <div className="rounded-2xl border border-[var(--app-paper-border)] bg-[var(--app-paper-bg)] px-4 py-3">
                    <dt className="text-[11px] uppercase tracking-[0.18em] text-[var(--app-ink-low)]">{pick(lang, '로그인 방식', 'Sign-in method')}</dt>
                    <dd className="mt-1 text-[15px] font-[700] text-[var(--app-ink-high)]">{providerLabel}</dd>
                  </div>
                  <div className="rounded-2xl border border-[var(--app-paper-border)] bg-[var(--app-paper-bg)] px-4 py-3">
                    <dt className="text-[11px] uppercase tracking-[0.18em] text-[var(--app-ink-low)]">{pick(lang, '가입일', 'Joined')}</dt>
                    <dd className="mt-1 text-[15px] font-[700] text-[var(--app-ink-high)]">{formatDate(profile?.created_at || user?.created_at, lang)}</dd>
                  </div>
                  <div className="rounded-2xl border border-[var(--app-paper-border)] bg-[var(--app-paper-bg)] px-4 py-3">
                    <dt className="flex items-center gap-2 text-[11px] uppercase tracking-[0.18em] text-[var(--app-ink-low)]">
                      <Mail size={12} />
                      <span>{pick(lang, '연결된 이메일', 'Connected email')}</span>
                    </dt>
                    <dd className="mt-1 break-all text-[13px] text-[var(--app-ink-high)]">{user?.email || '-'}</dd>
                  </div>
                </dl>
              </div>

              <div className="bg-[var(--app-paper-muted-bg)] border border-[var(--app-paper-border)] rounded-[1.5rem] p-5">
                <p className="text-[11px] uppercase tracking-[0.2em] text-[var(--app-ink-low)]">{pick(lang, '안내', 'Notes')}</p>
