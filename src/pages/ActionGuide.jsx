    ...
    <div className="max-w-3xl mx-auto">
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex items-center justify-between mb-5">
          <h1 className="text-lg font-semibold text-[#101114]">{t('action.practice')}: {l(guide.title, lang)}</h1>
        ...
        <div className="bg-white border border-[#dedee5] rounded-[12px] shadow-[0_4px_24px_rgba(0,0,0,0.03)] p-5 mb-4">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <p className="text-[14px] font-semibold text-[#101114]">{l(guide.title, lang)}</p>
              <p className="text-[11px] text-[#686b82]">{l(guide.subtitle, lang)}</p>
            </div>
          </div>
          <p className="text-[12px] text-[#686b82] leading-relaxed mb-5">{l(guide.description, lang)}</p>

          <div className="relative space-y-0">
            <div className="absolute left-[9px] top-3 bottom-3 w-px bg-[#dedee5]" />
            {guide.steps.map((step, i) => (
              <div key={i} className="relative flex gap-3 py-2.5">
                ...
                <div className="text-[12px] text-[#686b82] leading-relaxed">
                  {step.link ? (
                  ...
                  {step.note && <p className="text-[10px] text-[#9497a9] mt-0.5">{l(step.note, lang)}</p>}
                </div>
              </div>
            ))}
          </div>

          <button onClick={handleComplete} className={`mt-5 inline-flex items-center justify-center gap-2 font-semibold text-[12px] transition-colors ${actionDone ? 'rounded-[12px] bg-[#eef0f3] border border-[rgba(20,158,97,0.20)] text-[#026b3f] px-4 py-2.5' : 'rounded-[56px] bg-[#5741d8] text-white hover:bg-[#7132f5] px-4 py-2.5'}`}>
            {actionDone ? <><Check size={14} /> {t('action.completed')}</> : <>✓ {l(guide.title, lang)} {t('action.complete')}</>}
          </button>
        </div>

        {nextGuide && (
          <Link to={`/action/${nextGuide.id}`} className="block bg-white border border-[#dedee5] rounded-[12px] shadow-[0_4px_24px_rgba(0,0,0,0.03)] p-4">
            <div className="flex items-center gap-3">
              {(() => { const Icon = iconMap[nextGuide.icon]; return <div className="w-10 h-10 rounded-lg bg-[#eef0f3] flex items-center justify-center">{Icon ? <Icon size={20} className="text-[#9497a9]" /> : null}</div> })()}
              <div>
                <p className="text-[13px] font-medium text-[#101114]">{t('action.next')}: {l(nextGuide.title, lang)}</p>
                <p className="text-[11px] text-[#9497a9]">{t('action.nextStart')}</p>
              </div>
            </div>
          </Link>
        )}
        ...
        <div className="bg-[#f7f7f8] border border-[#dedee5] rounded-[12px] shadow-[0_4px_24px_rgba(0,0,0,0.03)] p-4 mt-4">
          <div className="flex items-center gap-1.5 mb-2">
            <Shield size={14} className="text-[#dd5b00]" />
            <p className="text-[12px] font-medium text-[#dd5b00]">{t('action.safety')}</p>
          </div>
          <ul className="space-y-1">
            {tips.map((tip, i) => <li key={i} className="text-[11px] text-[#686b82] leading-relaxed">• {tip}</li>)}
          </ul>
        </div>
      </motion.div>
    </div>
