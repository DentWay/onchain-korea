      ...
      <div className="max-w-7xl mx-auto">
        <div className="ok-readable-panel-soft p-6 md:p-8">
          ...
          >
            ...
            <p className="text-[16px] text-[var(--text-mid)] mt-4 leading-relaxed max-w-xl">
              {t('why.desc')}
            </p>
            <div className="mt-10 border-t border-[rgba(255,255,255,0.10)] pt-6">
              <p className="text-[11px] uppercase tracking-[0.2em] text-[var(--text-low)]">
                {lang === 'ko' ? '핵심 원칙' : 'Design principles'}
              </p>
              <ul className="mt-4 divide-y divide-[rgba(255,255,255,0.08)]">
                <li className="flex items-start gap-3 py-4 text-[14px] leading-relaxed text-[var(--text-mid)]">
                  <span className="mt-0.5 shrink-0 text-[12px] font-bold tracking-[0.18em] text-[#79AFFF]">01</span>
                  <span>{lang === 'ko' ? '이번 주에 필요한 것만 보여줍니다.' : 'Only the next required step stays visible.'}</span>
                </li>
                <li className="flex items-start gap-3 py-4 text-[14px] leading-relaxed text-[var(--text-mid)]">
                  <span className="mt-0.5 shrink-0 text-[12px] font-bold tracking-[0.18em] text-[#79AFFF]">02</span>
                  <span>{lang === 'ko' ? '실습과 퀴즈로 넘어가도록 읽기 흐름을 짭니다.' : 'Reading is designed to lead into quiz and practice immediately.'}</span>
                </li>
                <li className="flex items-start gap-3 py-4 text-[14px] leading-relaxed text-[var(--text-mid)]">
                  <span className="mt-0.5 shrink-0 text-[12px] font-bold tracking-[0.18em] text-[#79AFFF]">03</span>
