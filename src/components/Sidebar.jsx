...
export default function Sidebar({ onClose }) {
  ...
  return (
    <aside className="flex h-screen w-[236px] shrink-0 flex-col border-r border-[var(--app-divider)] bg-[var(--app-paper-bg)] p-3 text-[var(--app-ink-high)]">
      <div className="px-1 mb-2 flex items-start justify-between">
        <div>
          <Link to="/" className="block hover:opacity-80 transition-opacity">
            <BrandLockup surface={brandSurface} className="origin-left scale-[0.82]" />
          </Link>
          <div className="flex items-center gap-1.5 mt-2">
            <p className="text-[10px] text-[var(--app-ink-low)]">{sidebarTagline}</p>
          </div>
        </div>
        <button onClick={onClose} aria-label="Close menu / 메뉴 닫기" className="md:hidden p-1 rounded-lg hover:bg-[var(--app-soft-bg)] transition-colors mt-0.5">
          <X size={16} className="text-[var(--app-ink-low)]" />
        </button>
      </div>
      ...
      <nav aria-label="Main navigation" className="mt-4 flex-1 overflow-y-auto">
        {navItems.map((group) => (
          <div key={group.label}>
            <p className="mt-5 mb-1.5 px-2 text-[9px] uppercase tracking-[0.22em] text-[var(--app-ink-low)]">{group.label}</p>
            {group.items.map((item) => {
              ...
              const className = `mb-1 flex items-center gap-2.5 rounded-xl px-3 py-2 text-[12px] transition-all ${isActive ? 'bg-[rgba(87,65,216,0.08)] text-[#5741d8]' : 'text-[var(--app-ink-mid)] hover:bg-[var(--app-soft-bg)]'}`

              if (item.href) {
                return (
                  <a key={item.text} href={item.href} onClick={onClose} className={className}>
                    <Icon size={15} className={isActive ? 'text-[#5741d8]' : 'text-[var(--app-ink-low)]'} />
                    <span>{item.text}</span>
                  </a>
                )
              }
      ...
      <div className="mt-2 border-t border-[var(--app-divider)] pt-3">
        <div className="px-2 py-1">
          {isAdminWorkspace ? (
            <>
              <div className="rounded-2xl border border-[var(--app-paper-border)] bg-[var(--app-paper-muted-bg)] px-3 py-3">
                <p className="text-[10px] uppercase tracking-[0.18em] text-[var(--app-ink-low)]">{pick(lang, '관리자 세션', 'Admin Session')}</p>
                <p className="mt-1 text-[12px] font-semibold text-[var(--app-ink-high)]">
                  {adminAccessGranted ? pick(lang, '운영 콘솔 사용 가능', 'Console unlocked') : pick(lang, '관리자 잠금 상태', 'Console locked')}
                </p>
                <p className="mt-1 text-[10px] text-[var(--app-ink-mid)]">{displayName || user?.email || '-'}</p>
              </div>
              {supabaseEnabled && user && (
                <div className="pt-3 mt-3 border-t border-[var(--app-divider)]">
                  <div className="flex items-center justify-between">
                    <div className="min-w-0 mr-2">
                      <span className="text-[11px] text-[var(--app-ink-mid)] truncate block">{displayName}</span>
                      {isAdmin && <span className="text-[10px] text-[#5741d8]">Admin</span>}
                    </div>
                    <button type="button" onClick={handleSignOut} className="inline-flex items-center gap-1.5 text-[var(--app-ink-low)] hover:text-[var(--app-ink-mid)] transition-colors shrink-0" title={t('auth.signOut')} aria-label={t('auth.signOut')}>
                      <LogOut size={14} />
                  ...
                  {isAdmin && adminAccessGranted && (
                    <button
                      ...
                      className="mt-3 inline-flex items-center gap-1.5 text-[11px] text-[var(--app-ink-low)] hover:text-[var(--app-ink-mid)] transition-colors"
                    >
                      <LockKeyhole size={13} />
            ...
            <>
              <div className="flex items-center justify-between mb-1.5">
                <p className="text-[10px] text-[var(--app-ink-low)]">{t('sidebar.progress')}</p>
                <p className="text-[10px] text-[#5741d8] font-semibold tabular-nums">{overallProgress}%</p>
              </div>
              <div className="h-1.5 overflow-hidden rounded-full bg-[var(--app-track)]"><div className="h-full rounded-full bg-[#5741d8] transition-all duration-500" style={{ width: `${overallProgress}%` }} /></div>
              <p className="mt-2 text-[10px] text-[var(--app-ink-low)]">{t('common.week')} {activeWeek} {t('sidebar.weekProgress')}</p>
              {supabaseEnabled && user && (
                <div className="pt-3 mt-3 border-t border-[var(--app-divider)]">
                  <div className="flex items-center justify-between">
                    <div className="min-w-0 mr-2">
