    ...
    <div className="flex flex-col items-center gap-2" role="progressbar" aria-valuenow={animated} aria-valuemin={0} aria-valuemax={100} aria-label={label || 'Progress'}>
      <div className="relative" style={{ width: config.size, height: config.size }}>
        <svg width={config.size} height={config.size} className="-rotate-90" aria-hidden="true">
          <circle cx={config.size / 2} cy={config.size / 2} r={radius} fill="none" stroke="var(--app-track)" strokeWidth={config.stroke} />
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <span className={`${config.font} font-bold text-[var(--app-ink-high)] tabular-nums`}>{animated}%</span>
        </div>
      </div>
      {label && <p className="text-[11px] text-[var(--app-ink-low)]">{label}</p>}
    </div>
  )
}
